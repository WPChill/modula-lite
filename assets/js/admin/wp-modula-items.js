...
(function ($, modula) {
	var modulaItemsCollection = Backbone.Collection.extend({
		...
		initialize: function () {
			...
			this.listenTo(
				this,
				'remove',
				$.proxy(wp.Modula.Save.checkSave, wp.Modula.Save)
			);
			this.listenTo(
				this,
				'add',
	...
	var modulaItem = Backbone.Model.extend({
		...
		initialize: function (args) {
			...
			modula.Items =
				'undefined' === typeof modula.Items
					? new modula.items['collection']()
					: modula.Items;
			...
			// Create item HTML
			var view = new modula.items['view']({
				model: this,
				id: 'modula-item-' + this.get('id'),
			});
			this.set('view', view);
