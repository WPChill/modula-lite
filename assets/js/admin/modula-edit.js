  ...
  jQuery(window).on("load", function () {
    ...
    if (
      ...
      jQuery("html,body").animate(
        {
          scrollTop:
            jQuery(".wpchill-plans-table.wpchill-highlight").offset().top - 150
        },
        600
      );
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  button.addEventListener("click", async (event) => {
    ...
    try {
      ...
      if (
        undefined !== originalPostStatus &&
        "auto-draft" === originalPostStatus.value &&
        undefined !== postIdField &&
        undefined !== postIdField.value
      ) {
        const newUrl =
          modulaHelper.admin_url +
      ...
      if (
        undefined !== wp.Modula.watermark &&
        undefined !== wp.Modula.watermark.Conditions
      ) {
